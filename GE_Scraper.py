...
#Use your own discord tag under user-agent
headers = {'User-Agent': 'Merching Calc - @####'}
...
def Recipe(Inputs, QuantitiesIn, Outputs, QuantitiesOut, ExtraCost = 0):
    ...
    return(Inputs, LowList, Outputs, HighList, f(Profit(UnitValue - ExtraCost, UnitCost)))
...
def Calcs():
    ...
    Output.append(Recipe(["Masori mask (f)", "Masori chaps (f)", "Masori body (f)"], [1,1,1], "Masori armour set (f)", 1))
    Output.append(Recipe(["Inquisitor's plateskirt", "Inquisitor's hauberk", "Inquisitor's great helm"], [1,1,1], "Inquisitor's armour set", 1))
    Output.append(Recipe(["Ancestral hat", "Ancestral robe top", "Ancestral robe bottom"],[1,1,1],"Ancestral robes set",1))
    Output.append(Recipe(["Justiciar faceguard", "Justiciar chestguard", "Justiciar legguards"],[1,1,1],"Justiciar armour set",1))
    ...
    Output.append(Recipe(["Armadyl crossbow", "Nihil horn", "Nihil shard"], [1,1,250], "Zaryte crossbow", 1))
    Output.append(Recipe("Venator shard", 5, "Venator bow (uncharged)", 1))
    Output.append(Recipe(["Voidwaker blade", "Voidwaker hilt", "Voidwaker gem"],[1,1,1],"Voidwaker",1,500000))
    Output.append(Recipe(["Zamorakian spear"],[1],"Zamorakian hasta",1,150000))
    Output.append(Recipe(["Zamorakian hasta", "Hydra's claw"],[1,1],"Dragon hunter lance",1))
    Output.append(Recipe(["Kodai insignia", "Master wand"],[1,1],"Kodai wand",1))
    Output.append(Recipe(["Imbued heart", "Ancient essence"],[1,150000],"Saturated heart",1))

    Output.append(Recipe([["Thammaron's sceptre (u)","Thammaron's sceptre (au)"], "Skull of vet'ion"],[[1,1],1],[["Accursed sceptre (u)","Accursed sceptre (au)"]],[[1,1]]))
    Output.append(Recipe(["Craw's bow (u)", "Fangs of venenatis"],[1,1],"Webweaver bow (u)",1))
    Output.append(Recipe(["Viggora's chainmace (u)", "Claws of callisto"],[1,1],"Ursine chainmace (u)",1))

    Output.append(Recipe(["Spirit shield", "Holy elixir"],[1,1],"Blessed spirit shield",1))
    Output.append(Recipe(["Elysian sigil", "Blessed spirit shield"],[1,1],"Elysian spirit shield",1))
    Output.append(Recipe(["Spectral sigil", "Blessed spirit shield"],[1,1],"Spectral spirit shield",1))
    Output.append(Recipe(["Odium shard 1", "Odium shard 2", "Odium shard 3"],[1,1,1],"Odium ward",1))
    Output.append(Recipe(["Malediction shard 1", "Malediction shard 2", "Malediction shard 3"],[1,1,1],"Malediction ward",1))

    Output.append(Recipe(["Draconic visage", "Anti-dragon shield"],[1,1],"Dragonfire shield",1))
    Output.append(Recipe(["Skeletal visage", "Anti-dragon shield"],[1,1],"Dragonfire ward",1))
    Output.append(Recipe(["Wyvern visage", "Elemental shield"],[1,1],"Ancient wyvern shield",1))

    Output.append(Recipe(["Eternal crystal", "Infinity boots"],[1,1],"Eternal boots",1))
    Output.append(Recipe(["Pegasian crystal", "Ranger boots"],[1,1],"Pegasian boots",1))
    Output.append(Recipe(["Primordial crystal", "Dragon boots"],[1,1],"Primordial boots",1))

    Output.append(Recipe(["Zenyte shard", ["Uncut onyx", "Onyx"], "Cosmic rune", "Soul rune", "Blood rune", "Gold bar"], [1, [1,1], 1, 20, 20, 1], [["Necklace of anguish", "Amulet of torture", "Tormented bracelet", "Ring of suffering"]], [[1,1,1,1]]))
    ...
    Output['Output Price'] = Output['Output Price'].apply(lambda x: f(x))
    Output['Input Price'] = Output['Input Price'].apply(lambda x: f(x))
